...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?>>
		...
		<div class="hentry--post-thumbnail">
				<a href="<?php esc_url( the_permalink() ); ?>" class="hentry--thumbnail">
					...
					<?php if ( 'yes' === get_theme_mod( 'blog_date_label', 'yes' ) ) : ?>
						<time datetime="<?php esc_attr( the_time( 'c' ) ); ?>" class="meta-data--date date updated"><?php echo esc_attr( get_the_date() ); ?></time>
					<?php endif; ?>
				</a>
		</div>
	<?php endif; ?>
	...
	<div class="hentry--content">
		<?php if ( 'post' === get_post_type() && 'show' === get_theme_mod( 'blog_metadata', 'show' ) ) : ?>
			<div class="hentry--meta-data">
				...
				<?php if ( 'no' === get_theme_mod( 'blog_date_label', 'yes' ) ) : ?>
					...
					<span class="meta--date date updated"><?php the_time( get_option( 'date_format' ) ); ?></span>
				<?php endif; ?>
		...
		<?php the_title( sprintf( '<h2 class="hentry--title entry-title"><a href="%s">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
